...
def trajectory_feature_generation(path=None,
    ...
    for i, traj in enumerate(trajs):
        ...
        if (len(traj) > min_length):
            ...
            for p in traj:
                ...
                new_traj_record.append([p[0], p[1]])
            ...
            if inrange:
                ...
                if ((len(coor_traj) > 10) & (len(coor_traj) < 150)):
                    ...
                    traj_traj.append(new_traj_record)
                    traj_time_record.append(new_traj[0][-1])
    ...
    with open('./features/{}_traj_index'.format(fname), 'wb') as f:
        pickle.dump(traj_index, f)  # 筛选完之后的原始轨迹 beijing:9553,porto:620876
    ...
    with open('./features/{}_traj_coord'.format(fname), 'wb') as f:
        pickle.dump((trajs, max_len), f)  # traj_index之后映射成网格之后对应的经纬度
    all_trajs_grids_xy = []
    ...
    for i in trajs:
        for j in i:
            ...
            if y > max_y:
                max_y = y
    print(min_x, min_y, max_x, max_y)  # 网格的行列的边界
    ...
    with open('./features/{}_traj_grid'.format(fname), 'wb') as f:
        pickle.dump((all_trajs_grids_xy, max_len), f)  # 记录相对网格位置的x，y
